<p align="center">
  <img src="https://github.com/thompsonemerson/thompsonemerson/raw/master/cover-thompson.png" height="200"/>
</p>
<h1 align="center">Hi, I'm Emiliano.</h1> 

  </a>
  <a href="mailto:navarretehernan383@gmail.com">
    <img src="assets/Gmail-Dark.svg" alt="Gmail" width="50"/>
  </a>
</span>

<br>
<h2>about me</h2>

<p align="left">

Hi, I'm Emiliano Hernández Navarrete. I'm a 15-year-old high school student at CETIS No. 01 "Coronel Matilde Galicia Rioja." I'm passionate about programming and aspire to become an ethical hacker. My interest in technology really took off at the *Escuela Mexicana de Computación e Informática Avanzada*, where I earned a certification as a microcomputer operator.

That experience sparked my interest in tech, specifically Red Teaming—focusing on offensive security and attacking systems ethically. From there, I taught myself how to use Kali Linux, which opened the door to everything I know today. Along the way, I learned programming languages like C, C++, C#, Python, and Java, as well as basic web development (HTML, CSS, and JavaScript).

Rather than sticking to standard web development like most people, I was drawn to penetration testing, phishing simulations, and offensive security techniques. I've created several repositories to showcase everything I've learned and built.

<h2 >Currently Learning & Exploring</h2>

<p> 
    <img src="assets/Arch-Dark.svg" width="50"/>
    <img src="assets/Bash-Dark.svg" width="50"/>
    <img src="assets/C.svg" width="50"/>
    <img src="assets/CPP.svg" width="50"/>
    <img src="assets/CS.svg" width="50"/>
    <img src="assets/CSS.svg" width="50"/>
    <img src="assets/Debian-Dark.svg" width="50"/>
    <img src="assets/Git.svg" width="50"/>
    <img src="assets/Github-Dark.svg" width="50"/>
    <img src="assets/HTML.svg" width="50"/>
    <img src="assets/Java-Dark.svg" width="50"/>
    <img src="assets/JavaScript.svg" width="50"/>
    <img src="assets/Kali-Dark.svg" width="50"/>
    <img src="assets/Linux-Dark.svg" width="50"/>
    <img src="assets/Powershell-Dark.svg" width="50"/>
    <img src="assets/Python-Dark.svg" width="50"/>
</p>

<br>
<div id="proyectos">
<h2 >Repositories</h2>

<div id="github-container" style="display: flex; gap: 20px; align-items: center;"></div>

<p>
  <a href="https://github.com/Emiliano-HN/Z-Nexor" target="_blank">
    <img src="assets/Github-Nexor.png" alt="GitHub Nexor" width="50"/>
  </a>
  <a href="https://github.com/Emiliano-HN/Z-ATM" target="_blank">
    <img src="assets/GitHub-Cajero.png" alt="GitHub ATM" width="50"/>
  </a>
   <a href="https://github.com/Emiliano-HN/Z-AutoPayloadGen" target="_blank">
    <img src="assets/GitHub-AutoPayloadGen.png" alt="GitHub AutoPayloadGen" width="50"/>
  </a>
</p>

![footer](https://github.com/GovindSingh9447/GovindSingh9447/blob/main/WEBP/footer.webp)
